...
if (isset($_POST['type'])) {
  if ($_POST['type'] == 'create') {
    ...
    echo "<script>alert('Data pembayaran dengan ID $row[id] berhasil diperbarui')</script>";
    echo "<script>window.location='?url=pembayaran'</script>";
  }
}
?>
    ...
    <div class="card">
      ...
      <div class="card-body">
          ...
          <select class="form-control" id="peminjaman_id" name="peminjaman_id" required>
            ...
            <?php foreach ($list_peminjaman as $peminjaman): ?>
              <option value="<?= $peminjaman['id'] ?>" <?= getSafeFormValue($show_pembayaran, 'peminjaman_id') == $peminjaman['id'] ? 'selected' : '' ?>>
                <?= $peminjaman['nama_peminjam'] ?> - <?= $peminjaman['nama_armada'] ?>
              </option>
            <?php endforeach; ?>
          </select>
        </div>
        ...
        <div class="form-group">
          ...
          <input type="number" class="form-control" id="jumlah_bayar" name="jumlah_bayar" value="<?= getSafeFormValue($show_pembayaran, 'jumlah_bayar') ?>" required>
        </div>
      </div>
